...
description: Create, repair, validate, and refine native AnimFlow 2.2 lecture diagrams. Use when an AI agent must turn a technical teaching goal or supported Mermaid flowchart into an animated `.animflow` lesson; do not use for general slide decks or unsupported diagram kinds.
---

# AnimFlow Authoring

Produce valid native AnimFlow 2.2 source whose teaching sequence and layout are understandable without hidden editor state.
...
- Emit `animflow 2.2`. Every non-`say` scene statement, including nested `sequence` and `stagger`, has a document-unique `action <id>:` prefix.
...
- Prefer automatic flow layout. Add `position x <number> y <number>` only to express a requested composition or repair a verified layout problem; omit `pin` unless the coordinate must remain exact. Keep unpinned positions at least `nodeGap` apart so the compiler does not need to nudge them.
- Treat layout as source: never describe editor-only coordinates or assume a drag state will be published. After position edits, compile again and inspect diagnostics and generated geometry.
